...
const Product = () => {
  ...
  const generateOrderSummary = useCallback(() => {
    if (!productData) return '';
    let summaryParts = [];
    if (selectedSize) summaryParts.push(selectedSize.toLowerCase());
    if (selectedBase) summaryParts.push(selectedBase.toLowerCase());
    else summaryParts.push(productData.name.toLowerCase());
    if (selectedSide) summaryParts.push(`with ${selectedSide.toLowerCase()}`);
    if (selectedWrap) summaryParts.push('to wrap yourself');
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-gray-50 to-cyan-50/30 dark:from-gray-900 dark:to-cyan-900/10">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-cyan-500">Loading laptop details...</p>
        </div>
      </div>
    );
  }
              ...
              <div className="flex items-start mb-4">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-gray-600 text-sm">
                    Why is this important? These selections help us prepare your order exactly how you want it.
                  </p>
                </div>
              </div>
              <div className="bg-cyan-500/10 p-4 rounded-lg">
                ...
                <ul className="list-disc pl-5 text-gray-700 text-sm space-y-1">
                  {missingOptions.includes("Base") && (
                    <li>Select a <span className="font-medium">Base</span> option from the dropdown</li>
                  )}
                  {missingOptions.includes("Side") && (
                    <li>Choose your <span className="font-medium">Side</span> from the available options</li>
                  )}
                  {missingOptions.includes("size") && (
                    <li>Pick either a <span className="font-medium">Size</span> or the <span className="font-medium">Wrap</span> option</li>
                  )}
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <nav className="mb-8 text-sm text-gray-600" aria-label="Breadcrumb">
          <ol className="list-none p-0 inline-flex">
            ...
            <li className="flex items-center">
              <a href="/products" className="hover:text-cyan-500 transition-colors">Products</a>
              <svg className="w-3 h-3 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path>
              </svg>
            </li>
            <li className="flex items-center">
              <span className="text-cyan-500 font-medium">{productData.name}</span>
            </li>
          </ol>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-3 overflow-x-auto py-2">
              {productData.image.map((item, index) => (
                <div 
                  key={index}
                  className={`flex-shrink-0 cursor-pointer border-2 rounded-lg overflow-hidden ${mainImage === item ? 'border-cyan-500' : 'border-transparent'}`}
          ...
          <div className="flex-1">
            ...
            <div className="space-y-4 mb-6">
              ...
              {productData.variations?.base?.options?.length > 0 && (
                <div>
                  ...
                  {!selectedBase && missingOptions.includes("Base") && (
                    <p className="text-red-500 text-sm mt-1">Please select a base option</p>
                  )}
                </div>
              )}
              {/* Side Selector */}
              {productData.variations?.side?.options?.length > 0 && (
                <div>
                  ...
                  {!selectedSide && missingOptions.includes("Side") && (
                    <p className="text-red-500 text-sm mt-1">Please select a side option</p>
                  )}
                </div>
              )}
              {/* Size Selector */}
            ...
            <div className="bg-cyan-500/10 p-5 rounded-lg">
              <h3 className="gamer-font text-lg text-cyan-500 mb-2">Our Promise:</h3>
                  ...
                  <form onSubmit={handleReviewSubmit}>
                    ...
                    {!token && (
                      <p className="text-gray-600 mt-3">
                        You must be <a href="/login" className="text-cyan-500 hover:underline">logged in</a> to submit a review.
                      </p>
                    )}
                  </form>
                </div>
