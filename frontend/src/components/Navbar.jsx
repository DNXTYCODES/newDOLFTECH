...
import { FiSearch, FiUser, FiShoppingCart, FiMenu, FiSun, FiMoon } from 'react-icons/fi';
...
const Navbar = ({ toggleDarkMode, isDarkMode }) => {
    ...
    return (
        <header className="flex items-center justify-between py-5 font-medium bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 sm:px-6 lg:px-8">
            {/* Logo */}
                ...
                <ul className='hidden md:flex gap-8 text-base'>
                    <li>
                        <NavLink 
                            to='/' 
                            className={({isActive}) => 
                                `flex flex-col items-center ${isActive ? 'text-cyan-500 font-semibold' : 'text-gray-700 dark:text-gray-300 hover:text-cyan-500'}`
                            }
                        >
                            {({isActive}) => (
                                <>
                                    <span>HOME</span>
                                    <hr className={`w-2/4 h-[2px] mt-1 ${isActive ? 'bg-cyan-500' : 'bg-transparent'}`} />
                                </>
                            )}
                        </NavLink>
                    </li>
            ...
            <div className='flex items-center gap-5'>
                ...
                <Link to='/cart' className='relative text-gray-700 dark:text-gray-300 hover:text-cyan-500 transition-colors' aria-label="Shopping cart">
            ...
            {/* Mobile Menu */}
            <div 
                className={`fixed top-0 right-0 bottom-0 bg-white dark:bg-gray-900 w-full max-w-xs transition-transform duration-300 ease-in-out z-[100] ${visible ? 'translate-x-0' : 'translate-x-full'}`}
                aria-hidden={!visible}
            >
                <div className='flex flex-col h-full'>
                    <div className='flex justify-between items-center p-5 border-b border-gray-200 dark:border-gray-800'>
                        ...
                        >
                            <span className='text-lg font-medium text-gray-700 dark:text-gray-300'>Close</span>
                    ...
                    <nav className='flex-1 flex flex-col py-4' aria-label="Mobile navigation">
                        <NavLink 
                            onClick={() => setVisible(false)} 
                            className={({isActive}) => 
                                `py-4 px-6 text-lg ${isActive ? 'text-cyan-500 font-semibold bg-cyan-50 dark:bg-cyan-900/20' : 'text-gray-700 dark:text-gray-300'}`
                            } 
                            to='/'
                        >
                            HOME
